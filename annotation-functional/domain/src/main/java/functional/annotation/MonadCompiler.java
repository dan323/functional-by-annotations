...
public class MonadCompiler implements Compiler {
    ...
    private boolean validateMonad(TypeElement element, DeclaredType iface) {
        ...
        for (var elem : element.getEnclosedElements()) {
            if (elem.getKind().equals(ElementKind.METHOD) && elem.getModifiers().contains(Modifier.STATIC) && elem.getModifiers().contains(Modifier.PUBLIC)) {
                if (elem.getSimpleName().toString().equals("flatMap") && checkIfFlatmap((ExecutableElement) elem, iface)) {
                    successFlatmap = true;
                } else if (elem.getSimpleName().toString().equals("join") && checkIfJoin((ExecutableElement) elem, iface)) {
                    successJoin = true;
                } else if (elem.getSimpleName().toString().equals("pure") && checkIfPure((ExecutableElement) elem, iface)) {
                    successPure = true;
                } else if (elem.getSimpleName().toString().equals("map") && checkIfMap((ExecutableElement) elem, iface)) {
                    successMap = true;
                } else if (elem.getSimpleName().toString().equals("fapply") && checkIfFapply((ExecutableElement) elem, iface)) {
                    successFapply = true;
                } else if (elem.getSimpleName().toString().equals("liftA2") && checkIfLiftA2((ExecutableElement) elem, iface)) {
                    successLiftA2 = true;
                }
