...
public class FunctorCompilerTest {
    ...
    public void eitherFunctor(){
        ...
        args.addAll(0, List.of("-processor", FunctionalCompiler.class.getName(), "-p", "../annotation-definitions/target/annotation-definitions-1.2-SNAPSHOT.jar", "--add-modules", "functional.annotations"));
    ...
    @Test
    public void functorWrongName() {
        List<String> args = Stream.of("src/test/java/com/dan323/functional/annotation/compiler/functor/FunctorWrongName")
                .map(s -> Paths.get(s + ".java").toAbsolutePath().toString())
                .collect(Collectors.toList());
        args.addAll(0, List.of("-processor", FunctionalCompiler.class.getName(), "-p", "../annotation-definitions/target/annotation-definitions-1.2-SNAPSHOT.jar", "--add-modules", "functional.annotations"));
        String[] flags = args.toArray(new String[4]);
    ...
    @Test
    public void functorWrongReturn() {
        List<String> args = Stream.of("src/test/java/com/dan323/functional/annotation/compiler/functor/FunctorWrongReturnType")
                .map(s -> Paths.get(s + ".java").toAbsolutePath().toString())
                .collect(Collectors.toList());
        args.addAll(0, List.of("-processor", FunctionalCompiler.class.getName(), "-p", "../annotation-definitions/target/annotation-definitions-1.2-SNAPSHOT.jar", "--add-modules", "functional.annotations"));
        String[] flags = args.toArray(new String[4]);
