...
public class FunctionFromTest {
    ...
    public void functionFromFunctor() {
        var fun = FunctionFrom.<String>getInstance().map(Integer::parseInt, String::valueOf);
    ...
    public void functionFromPure() {
        var fun = FunctionFrom.<Integer>getInstance().pure(9);
    ...
    public void functionFromFapply() {
        var fun = FunctionFrom.<Integer>getInstance().fapply(x -> (y -> y - x), u -> u * 2);
    ...
    public void functionFromLiftA2() {
        var fun = FunctionFrom.<Integer>getInstance().liftA2((a, b) -> a * b, u -> u * 3, v -> v * 2);
    ...
    public void functionFromJoin() {
        var fun = FunctionFrom.<Integer>getInstance().join(x -> (y -> x + y * 7));
    ...
    public void functionFromFlatMap(){
        var fun = FunctionFrom.<Integer>getInstance().flatMap(x -> (y-> Math.max(x,y)), y -> y*2-1);
