...
public final class FoldableUtil {
    ...
    public static <G extends IFoldable<FW>, FW extends F, F, FA extends F, A> A fold(G foldable, IMonoid<A> monoid, Class<F> fClass, FA a) {
    ...
    public static <G extends IFoldable<FW>, FW extends F,F,FA extends F, A, M> M foldMap(G foldable, IMonoid<M> monoid, Class<F> fClass, Function<A,M> function, FA base){
        return FunctionalUtil.foldableFoldMap(foldable, monoid, fClass, function, base)
                .orElseThrow(() -> new IllegalArgumentException("The foldable is not correctly implemented."));
    }

    public static <G extends IFoldable<FW>, FW extends F,F,FA extends F, A, B> B foldr(G foldable, Class<F> fClass, BiFunction<A,B,B> function, B b, FA fa){
