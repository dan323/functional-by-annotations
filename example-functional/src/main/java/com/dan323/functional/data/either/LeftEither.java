...
public final class LeftEither<R> implements IMonad<Either<?, R>> {
    ...
    public static <A, B, R> Either<B, R> map(Either<A, R> base, Function<A, B> mapping) {
        return base.either(((Function<B, Either<B, R>>) Either::left).compose(mapping), Either::right);
    }

    public static <B, R> Either<B, R> pure(B r) {
        return Either.left(r);
    }

    public static <A, B, R> Either<B, R> flatMap(Function<A, Either<B, R>> f, Either<A, R> base) {
        return base.either(f, Either::right);
    }

    public static <A, B, R> Either<B, R> fapply(Either<Function<A, B>, R> fEither, Either<A, R> base) {
        return fEither.either(f -> map(base, f), Either::right);
    }

    public static <A, R> Either<A, R> join(Either<Either<A, R>, R> eitherEither) {
